...
const _producttypeentity = require("./entities/product-type.entity");
const _taxclassentity = require("./entities/tax-class.entity");
...
let ProductsService = class ProductsService {
            ...
            const defaultTypes = [
                {
                    name: 'Physical Good',
                    code: 'physical',
                    description: 'Tangible products that require inventory tracking and shipping.'
                },
                {
                    name: 'Professional Service',
                    code: 'service',
                    description: 'Internal or external services, maintenance, or consulting.'
                },
                {
                    name: 'Digital Product',
                    code: 'digital',
                    description: 'Software, licenses, or downloadable assets.'
                }
            ];
    ...
    async getTaxClasses() {
        return this.taxClassRepository.find({
            order: {
                name: 'ASC'
            }
        });
    ...
    async updateTaxClass(id, data) {
        ...
        return this.taxClassRepository.findOne({
            where: {
                id
            }
        });
    ...
    async findAllPaginated(page = 1, limit = 5, search, categoryId, status, type) {
    ...
    async archive(id) {
        const product = await this.findOne(id);
        product.status = _productentity.ProductStatus.ARCHIVED;
        return this.productRepository.save(product);
    }
    async duplicate(id) {
        ...
        const duplicatedProduct = this.productRepository.create({
            ...
            status: _productentity.ProductStatus.DRAFT
        });
    ...
    constructor(productRepository, categoryRepository, unitRepository, pricingModelRepository, variantRepository, priceBookRepository, priceBookItemRepository, brandRepository, inventoryRepository, mediaRepository, attributeRepository, productTypeRepository, taxClassRepository){
...
ProductsService = _ts_decorate([
    ...
    _ts_param(11, (0, _typeorm.InjectRepository)(_producttypeentity.ProductType)),
    _ts_param(12, (0, _typeorm.InjectRepository)(_taxclassentity.TaxClass)),
